...
public class ZHangmanWebHook extends HttpServlet{
    ...
    //private static final String youtubeURL = "https://www.googleapis.com/youtube/v3/search?part=id&q=XXXX&type=video&key=AIzaSyAs4apfcZCcf63CGULKnRb9Y_TSj5KkYD4"; 
     public static final String fbProfileFetchURL = "https://graph.facebook.com/v2.8/XXXX?access_token=EAAY4CNnNWnsBAIvbhfebqoSIGKzAU0nw07OwtU6yZBXZAB276IfJQQIZAlARmOzNTcYfHwhBZAH6wiDBZBp50wZCDUyuDVWF1BTxXQZC31gguINwbwm5C2CmhnDxWuUGqQ1dYZACP8QAIunSGOZA8Us3nftn82r0xkhUUTcsoH1UJsQZDZD"; 
    /***************************************/
           
     /******* API AI CALL *********/
     private static final String API_AI_URL = "https://api.api.ai/v1/query?v=20150910";
     private static final String ACCESS_CODE = "ce17cf2da1434a5cb8d3a7310496982e";
     public static final HttpPost api_ai_http_post = new HttpPost(API_AI_URL);
				...
				if (answer.equals(input)){
					...
					ArenaCache.getInstance().saveData(sd);
                                ...
                                if (!input.contains("SCOREBOARD") && !input.contains("ABOUT GAME") && !input.contains("SHARE")){
									   ...
									   try {
                                                                               String str_json = "{ query: \"XXXXXXXX\", lang: \"en\", sessionId: \"somerandomthing\" }";
                                                                               input = input.replaceAll("\"","");
                                                                               str_json = str_json.replace("XXXXXXXX",input);
                                                                     
										    HttpEntity entity = new ByteArrayEntity(str_json.getBytes("UTF-8"));
										    api_ai_http_post.setHeader("Content-Type", "application/json; charset=utf-8");
										    api_ai_http_post.setHeader("Authorization", "Bearer " + ACCESS_CODE);
										    api_ai_http_post.setEntity(entity);
					                        HttpResponse response = client.execute(api_ai_http_post);
					                        String result = EntityUtils.toString(response.getEntity());
                                                                //System.out.println(result);
					                        JSONObject obj = new JSONObject(result);
					                        String speech = "\uD83D\uDC81 ";
                                                                speech = speech + (String) ((JSONObject) ((JSONObject) obj
					                                        .get("result")).get("fulfillment")).get("speech");
					                        System.out.println("INTELLIGENT RESPONSE AGAINST :"+input+" WAS "+ speech);
					                        ignoredMsg = speech;
									   }catch(Exception e){
										   e.printStackTrace();
										   ignoredMsg = "FYI, we may ignore your previous input for this game.";
									   }
...
private static void sendNotification(String sender, String text){
    Recipient recipient = new Recipient();
    recipient.setId(sender);
    com.zeek.fbbot.pojo.btnmenu.Message msg = new com.zeek.fbbot.pojo.btnmenu.Message();
    com.zeek.fbbot.pojo.btnmenu.Attachment attachment = new com.zeek.fbbot.pojo.btnmenu.Attachment();
